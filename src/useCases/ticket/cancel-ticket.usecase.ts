...
export default class CancelTicketUseCase implements ICancelTicketUseCase {
  ...
  async execute(ticketId: string, cancelCount: number): Promise<void> {
    const ticket = await this._ticketRepository.findOneWithPopulate({
      ticketId,
    });
    ...
    if (cancelCount > totalCount) {
      throw new CustomError(
        "Cancel count exceeds total ticket count",
    ...
    const eventDetails = await this._eventRepository.findOne({
      eventId: ticket.eventId,
    });

    /** Step 1: Refund to client */
    const clientWallet = await this._walletRepository.updateWallet(
      ticket.clientId,
      clientRefund
    );
    if (!clientWallet) {
      throw new CustomError(
        "Failed to update client wallet",
    ...
    const clientTransaction: ITransactionsEntity = {
      ...
      relatedTitle:  `Ticket Refund: ${eventDetails?.title || "an event"}`
    };

    const savedClientTx = await this._transactionRepository.save(
      clientTransaction
    );
    if (!savedClientTx) {
      throw new CustomError(
        "Failed to log client transaction",
        HTTP_STATUS.INTERNAL_SERVER_ERROR
      );
    }
    /** Step 2: Deduct from vendor */
    const vendorId = ticket.vendorId;
    const vendorWallet = await this._walletRepository.reduceMoney(
      vendorId,
      vendorShare
    );
    if (!vendorWallet) {
      throw new CustomError(
        "Failed to deduct vendor wallet",
        HTTP_STATUS.INTERNAL_SERVER_ERROR
      );
    }
    ...
    const vendorTransaction: ITransactionsEntity = {
      ...
      relatedTitle: `Refund for: ${eventDetails?.title || "an event"}`,
    };

    const savedVendorTx = await this._transactionRepository.save(
      vendorTransaction
    );
    if (!savedVendorTx) {
      throw new CustomError(
        "Failed to log vendor transaction",
        HTTP_STATUS.INTERNAL_SERVER_ERROR
      );
    }
    ...
    if (ticket.ticketCount === 0) {
      ...
      if (eventDetails?.attendees && Array.isArray(eventDetails.attendees)) {
        eventDetails.attendees = eventDetails.attendees.filter(
          (attendeeId) => attendeeId !== ticket.clientId
        );
      }
    } else {
    ...
    eventDetails.ticketPurchased = Math.max(
      0,
      eventDetails.ticketPurchased! - cancelCount
    );
    eventDetails.attendeesCount = Math.max(
      0,
      eventDetails.attendeesCount - cancelCount
    );
